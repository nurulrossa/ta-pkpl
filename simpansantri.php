...
$niserr = $emailErr = $genderErr = $websiteErr = "";
$NIS = $email = $gender = $comment = $website = "";
...
<div class="container">
	...
	<form method="POST" action="">
		<table class="table">
			...
			<tr>
				<td>Nama Santri</td><td><input class="form-control" type="text" name="Nama"></td>
			</tr>
			<td>Kelas</td>
				<td>
					<select name="kd_kelas">
						...
						<?php  
					    include "connect.php";
					    $koneksi = mysqli_query($koneksi,"SELECT * FROM kelas ORDER BY nama_kls");
					    foreach ($koneksi as $data) {
					  ?>
					  <option class="form-control" value=<?php echo $data['kd_kelas'] ?>><?php echo $data['nama_kls']; ?></option>
  					<?php } ?>
					</select>
				</td>
			</tr>
...
if($_SERVER['REQUEST_METHOD']=='POST'){
	...
	if($NIS==''){
		echo "<span style='color:red'>Form Belum Lengkap!!!</span>";
	}elseif ($Nama=='') {
